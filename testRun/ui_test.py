    ...
    def getTestSiutes(self):
        try:
            ...
            for ids in all_id:
                ...
                addUpdateDel('insert into report (case_name,type,product,status,Screenshot,spenttime,error,username,create_date) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)',[res['case_name'],'ui',str(ids[1]),res['status'],res['Screenshot'],res['spenttime'],res['error'],self.username,time.strftime('%Y-%m-%d %X', time.localtime(time.time()))])
            ...
            cases_report = [dict(case_name=row[0], type=row[1], product=row[2],status=row[3], Screenshot=row[4],  spenttime=row[5], error=row[6], username=row[7]) for row in cur]
